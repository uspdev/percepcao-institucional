<div>
    ...
    <div class="pl-6 pt-3">
        ...
        <div>
            @if (isset($disciplina))
                <span class="bold">Total de respostas:</span> {{ $this->getRespostas($grupo['id'], $questao['id'], $percepcao->id, $disciplina->id)['totalDeRespostas'] }}
            @endif
            @if (isset($coordenador))
                <span class="bold">Total de respostas:</span> {{ $this->getRespostas($grupo['id'], $questao['id'], $percepcao->id, null, $coordenador->id)['totalDeRespostas'] }}
            @endif
        </div>
    </div>
    <div class="chart-container pt-2">
        @if (isset($disciplina))
            <canvas id="myChart-{{ $percepcao->id }}-{{ $grupo['id'] }}-{{ $questao['id'] }}-disciplina-{{ $disciplina->id ?? '' }}"></canvas>
        @elseif (isset($coordenador))
            <canvas id="myChart-{{ $percepcao->id }}-{{ $grupo['id'] }}-{{ $questao['id'] }}-coordenador-{{ $coordenador->id ?? '' }}"></canvas>
        @else
    ...
    <script>
        ...
        @if (isset($disciplina))
            canvasId = 'myChart-{{ $percepcao->id }}-{{ $grupo['id'] }}-{{ $questao['id'] }}-disciplina-{{ $disciplina->id ?? '' }}';
        @endif

        @if (isset($coordenador))
            canvasId = 'myChart-{{ $percepcao->id }}-{{ $grupo['id'] }}-{{ $questao['id'] }}-coordenador-{{ $coordenador->id ?? '' }}';
        @endif
